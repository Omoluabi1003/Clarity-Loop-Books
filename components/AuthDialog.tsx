...
export function AuthDialog({ mode, accounts, onClose, onAuthenticated, onModeChange }: Props) {
  ...
  const isSignup = mode === "signup" && !recovering;
  ...
  return (
    <div className="auth-overlay" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section className={`auth-dialog auth-screen ${isSignup ? "signup-screen" : "signin-screen"} ${recovering ? "forgot-password-screen" : ""} studio-dark-surface`} role="dialog" aria-modal="true" aria-labelledby="auth-title">
        <button className="auth-close" type="button" aria-label="Close account dialog" onClick={onClose}><X size={18} /></button>
        <div className="auth-brand"><BrandLogo priority /></div>
        <div className="auth-heading">
          <span className="auth-icon"><UserRound size={20} /></span>
          <p className="eyebrow">{recovering ? "ACCOUNT RECOVERY" : isSignup ? "YOUR PRIVATE STUDIO" : "WELCOME BACK"}</p>
          <h2 id="auth-title">{recovering ? "Reset your password." : isSignup ? "Create your account." : "Sign in to your studio."}</h2>
          <p>{recovering ? "Enter the email connected to your studio account to begin recovery." : isSignup ? "Keep a recognizable studio identity on this browser and return to your writing workspace." : "Continue with the account you created on this browser."}</p>
        </div>
        ...
        <form onSubmit={submit}>
          ...
          {!recovering && <label><span>Password</span><div className="auth-input"><LockKeyhole size={17} /><input type={showPassword ? "text" : "password"} autoComplete={isSignup ? "new-password" : "current-password"} value={password} onChange={(event) => setPassword(event.target.value)} placeholder={isSignup ? "At least 8 characters" : "Your password"} /><button type="button" aria-label={showPassword ? "Hide password" : "Show password"} onClick={() => setShowPassword((current) => !current)}>{showPassword ? <EyeOff size={17} /> : <Eye size={17} />}</button></div></label>}
          ...
          {recoverySent && <p className="auth-success" role="status">Recovery is ready for {email}. This browser-only beta does not send email yet; server-backed recovery is recommended before launch.</p>}
          <button className="auth-submit" type="submit" disabled={submitting}>{recovering ? "Continue recovery" : submitting ? "Opening your studio…" : isSignup ? "Create account" : "Sign in"}</button>
        </form>

        {recovering ? <p className="auth-switch">Remembered your password? <button type="button" onClick={() => { setRecovering(false); setRecoverySent(false); setError(""); }}>Return to sign in</button></p> : <>
          {!isSignup && <p className="auth-switch auth-recovery-link"><button type="button" onClick={() => { setRecovering(true); setError(""); }}>Forgot password?</button></p>}
