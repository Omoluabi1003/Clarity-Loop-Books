...
import { BookOpenText, CircleHelp, LogIn, LogOut, Menu, PenLine, Rocket, Sparkles, UserPlus, UserRound, X } from "lucide-react";
...
import { LanguageSelector } from "./i18n/I18nProvider";
...
export function BookStudio() {
  ...
  const restoreBook = (book: Book) => requireAuthentication(() => {
    const next = booksRef.current.map((item) => item.id === book.id ? { ...item, deletedAt: null, archivedAt: null, status: item.chapters.length ? "drafting" as const : "blueprint" as const, updatedAt: new Date().toISOString() } : item);
    booksRef.current = next; setBooks(next); saveNow(next); setToast(`“${book.title}” restored to your writing desk.`); window.setTimeout(() => setToast(""), 3000);
  });
