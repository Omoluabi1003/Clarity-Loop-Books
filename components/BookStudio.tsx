...
import { BarChart3, BookOpenText, Languages, LogIn, LogOut, Menu, PenLine, Rocket, Sparkles, UserPlus, UserRound, X } from "lucide-react";
...
import { AuthorSuccessHub } from "./AuthorSuccessHub";
import { useI18n } from "./i18n/I18nProvider";
import { LOCALE_LABELS, SUPPORTED_LOCALES, type Locale } from "@/lib/i18n/config";

type View = "workspace" | "blueprint" | "chapters" | "author_success";
...
export function BookStudio() {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      ...
      const savedSession = parseSession(localStorage.getItem(AUTH_SESSION_KEY), savedAccounts);
      setAccounts(savedAccounts);
      setAuthUser(savedSession);
      setAuthMode(null);
  ...
  const requireAuthentication = (action: () => void) => {
    if (!authUser) {
      setAuthMode(accounts.length ? "signin" : "signup");
      setMobileMenu(false);
      return;
    }
    action();
  };
  const openBook = (book: Book, target: "blueprint" | "chapters") => requireAuthentication(() => { setActiveBookId(book.id); setView(target); window.scrollTo(0, 0); });
  const openAuthorSuccess = (book?: Book) => requireAuthentication(() => { const target = book || books.find((item) => !item.deletedAt); if (!target) { openWizard(); return; } setActiveBookId(target.id); setView("author_success"); setMobileMenu(false); window.scrollTo(0, 0); });
  const openWizard = (template?: BookTemplate) => requireAuthentication(() => { setCreationPath("start_from_idea"); setSelectedTemplate(template ?? null); setWizardOpen(true); });
  const openCreationPath = (path: CreationPathId) => requireAuthentication(() => { setCreationPath(path); setSelectedTemplate(null); setWizardOpen(true); });
  const requestDelete = (book: Book) => requireAuthentication(() => setBookToDelete(book));
  ...
  const renameBook = (book: Book, title: string) => { updateBook({ ...book, title }); setToast("Book renamed."); window.setTimeout(() => setToast(""), 2500); };
  const duplicateBook = (book: Book) => requireAuthentication(() => {
    const now = new Date().toISOString();
    const id = `book-${Date.now()}`;
    const duplicate = hydrateBook({ ...book, id, title: `${book.title} — Copy`, status: "drafting", createdAt: now, updatedAt: now, archivedAt: null, exportHistory: [], chapters: book.chapters.map((chapter, index) => ({ ...chapter, id: `${id}-chapter-${index + 1}`, bookId: id, locked: false })) });
    const next = [duplicate, ...booksRef.current]; booksRef.current = next; setBooks(next); saveNow(next); setToast("Book duplicated. Your original is unchanged."); window.setTimeout(() => setToast(""), 3000);
  });
  const archiveBook = (book: Book) => requireAuthentication(() => {
    const next = booksRef.current.map((item) => item.id === book.id ? { ...item, archivedAt: new Date().toISOString(), updatedAt: new Date().toISOString() } : item);
    booksRef.current = next; setBooks(next); saveNow(next); setToast("Book archived and removed from your writing desk."); window.setTimeout(() => setToast(""), 3000);
  });
  ...
  const authenticationRequired = !authUser && authMode !== null;
  const authenticationGate = authenticationRequired && authMode && <AuthDialog mode={authMode} accounts={accounts} dismissible={false} onClose={() => undefined} onModeChange={setAuthMode} onAuthenticated={finishAuthentication} />;

  if (activeBook && view === "author_success") return <div className="auth-gate-shell"><div className={`auth-gated-content${authenticationRequired ? " is-locked" : ""}`} aria-hidden={authenticationRequired}><AuthorSuccessHub book={activeBook} onBack={goWorkspace} onSave={updateBook} />{projectManagementOverlays}</div>{authenticationGate}</div>;
  if (activeBook && view === "blueprint") return <div className="auth-gate-shell"><div className={`auth-gated-content${authenticationRequired ? " is-locked" : ""}`} aria-hidden={authenticationRequired}><BlueprintView book={activeBook} onBack={goWorkspace} onStartWriting={() => setView("chapters")} onChange={updateBook} onDelete={() => requestDelete(activeBook)} />{projectManagementOverlays}{exportOpen && <ExportCenter book={activeBook} onClose={() => setExportOpen(false)} onExported={recordExport} />}</div>{authenticationGate}</div>;
  if (activeBook && view === "chapters") return <div className="auth-gate-shell"><div className={`auth-gated-content${authenticationRequired ? " is-locked" : ""}`} aria-hidden={authenticationRequired}><ChapterStudio book={activeBook} lastSaved={lastSaved} onSave={() => saveNow()} onBack={goWorkspace} onBlueprint={() => setView("blueprint")} onChange={updateBook} onExport={() => setExportOpen(true)} onAuthorSuccess={() => setView("author_success")} onDelete={() => requestDelete(activeBook)} />{projectManagementOverlays}{exportOpen && <ExportCenter book={activeBook} onClose={() => setExportOpen(false)} onExported={recordExport} />}</div>{authenticationGate}</div>;
  return <div className="auth-gate-shell"><div className={`auth-gated-content${authenticationRequired ? " is-locked" : ""}`} aria-hidden={authenticationRequired}><div className="app-shell os-app-shell"><header className="main-header os-main-header page-shell"><button className="brand" onClick={goWorkspace}><BrandMark /><span><strong>Clarity Loop</strong><small>AI AUTHOR OS</small></span></button><nav id="primary-navigation" className={mobileMenu ? "open" : ""}><button className="active" onClick={() => { setMobileMenu(false); document.getElementById("workspace")?.scrollIntoView({ behavior: "smooth" }); }}><PenLine size={16} /> Workspace</button><button onClick={() => { setMobileMenu(false); document.getElementById("books")?.scrollIntoView({ behavior: "smooth" }); }}><BookOpenText size={16} /> Books</button><button onClick={() => { setMobileMenu(false); document.getElementById("intelligence")?.scrollIntoView({ behavior: "smooth" }); }}><BarChart3 size={16} /> Intelligence</button><button onClick={() => openAuthorSuccess()}><Rocket size={16} /> Publishing</button><button onClick={() => authUser ? setMobileMenu(false) : openAuth(accounts.length ? "signin" : "signup")}><UserRound size={16} /> Account</button><div className="mobile-language-control" data-i18n-ignore><Languages size={16} /><label htmlFor="mobile-language">Language</label><select id="mobile-language" value={locale} onChange={(event) => setLocale(event.target.value as Locale)}>{SUPPORTED_LOCALES.map((item) => <option key={item} value={item}>{LOCALE_LABELS[item]}</option>)}</select></div><div className="mobile-account-actions" aria-label="Account actions">{authUser ? <><span className="mobile-account-chip" title={authUser.email}><UserRound size={16} /><span><small>Signed in as</small>{authUser.name}</span></span><button className="auth-nav-button auth-signout" onClick={signOut}><LogOut size={16} /> Sign out</button></> : <><p>Save your studio and return to your books on this device.</p><div><button className="auth-nav-button" onClick={() => openAuth("signin")}><LogIn size={16} /> Sign in</button><button className="auth-nav-button auth-create" onClick={() => openAuth("signup")}><UserPlus size={16} /> Sign up</button></div></>}</div></nav><div className="header-action"><div className="auth-actions header-auth-actions">{authUser ? <><span className="account-chip" title={authUser.email}><UserRound size={15} /><span><small>Signed in</small>{authUser.name}</span></span><button className="auth-nav-button auth-signout" onClick={signOut}><LogOut size={15} /> Sign out</button></> : <button className="auth-nav-button" onClick={() => openAuth("signin")}><LogIn size={15} /> Sign in</button>}</div><button className="new-book-nav" onClick={() => openWizard()}><Sparkles size={15} /> Start Your Book</button><button className="menu-button" type="button" aria-label={mobileMenu ? "Close navigation" : "Open navigation"} aria-expanded={mobileMenu} aria-controls="primary-navigation" onClick={() => setMobileMenu(!mobileMenu)}>{mobileMenu ? <X /> : <Menu />}</button></div></header><AuthorWorkspace books={books} onOpen={openBook} onCreate={openWizard} onCreatePath={openCreationPath} onDelete={requestDelete} onAuthorSuccess={openAuthorSuccess} onRename={renameBook} onDuplicate={duplicateBook} onArchive={archiveBook} /><footer className="studio-dark-surface"><div className="page-shell footer-inner"><div className="brand brand-light"><BrandMark /><span><strong>Clarity Loop</strong><small>AI AUTHOR OPERATING SYSTEM</small></span></div><p>Developed by <strong>ETL GIS Consulting LLC</strong></p><small>© ETL GIS Consulting LLC. All rights reserved.</small></div></footer><BetaFeedbackPanel books={books} onSubmit={submitFeedback} />{projectManagementOverlays}{wizardOpen && <NewBookWizard key={selectedTemplate?.id ?? "blank"} initialTemplate={selectedTemplate} initialPath={creationPath} onClose={() => { setWizardOpen(false); setSelectedTemplate(null); }} onCreate={createBook} />}</div></div>{authenticationGate}</div>;
}
