...
export function buildLeaderboardScores(rows: SalesReportRow[] = sampleSalesRows, filters: Partial<LeaderboardFilters> = {}): LeaderboardScore[] {
  ...
  return [...groups.entries()].map(([key, group]) => {
    ...
    const base = { entityId: crypto.createHash("sha1").update(key).digest("hex").slice(0, 10), entityName, entityType, badge: badgeForRows(group), window, verifiedUnitsSold: units(verified), estimatedUnitsSold: units(estimated), verifiedRevenue: revenue(verified), estimatedRevenue: revenue(estimated), salesGrowthVelocity: clamp(50 + units(group) / 260), reviewGrowthVelocity: clamp(42 + group.length * 11), categoryRankMovement: clamp(46 + units(group) / 340), campaignRoi: clamp(spend ? ((revenue(group) - spend) / spend) * 18 : 55), authorSatisfaction: clamp(group.reduce((sum, row) => sum + (row.authorSatisfaction ?? 70), 0) / group.length), sourceType: verified.length ? "verified" : group[0].sourceType, sourceName: group.map((row) => row.sourceName).join(" + "), confidenceLevel: confidenceForSource(verified.length ? "verified" : group[0].sourceType), collectedAt: group[0].collectedAt, verificationStatus: status, evidence: group.map((row) => `${row.verificationStatus.replaceAll("_", " ")}: ${row.sourceName}`) } satisfies Omit<LeaderboardScore, "claritySalesRankScore" | "includedInTopRankings">;
