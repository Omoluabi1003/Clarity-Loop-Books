...
test("the requested public logo path reuses the existing official binary without duplicating it", () => {
  ...
  const logo = readFileSync(publicLogo);
  assert.deepEqual([...logo.subarray(0, 8)], [137, 80, 78, 71, 13, 10, 26, 10]);
});

test("global metadata uses the canonical logo for share, browser, Apple, and install branding", () => {
  ...
  assert.doesNotMatch(layout, /clarity-loop-social-card|icons\/favicon|apple-touch-icon/);
