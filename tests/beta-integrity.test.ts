...
test("creation path catalog exposes all seven tailored publishing workflows", () => {
  ...
  assert.equal(new Set(CREATION_PATHS.map((path) => path.firstStepTitle)).size, 7);
  assert.deepEqual(CREATION_PATH_CONFIG.fiction_book.fields.map((field) => field.name), ["storyTitle", "authorName", "genre", "mainCharacter", "centralConflict", "setting", "emotionalPromise"]);
  assert.ok(CREATION_PATH_CONFIG.nonfiction_book.fields.some((field) => field.name === "frameworkOrMethod"));
  assert.ok(CREATION_PATH_CONFIG.upload_manuscript.fields.some((field) => field.type === "file"));
