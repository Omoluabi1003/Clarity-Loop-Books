...
interface ChapterRequest { bookTitle: string; tone: string; audience: string; chapter: Chapter; action?: "write" | "rewrite" | "expand" | "shorten" }

export async function POST(request: Request) {
  const body = (await request.json()) as ChapterRequest;
  if (!body.chapter || !body.bookTitle) return NextResponse.json({ error: "Book and chapter details are required." }, { status: 400 });
