...
export async function POST(request: Request) {
  try {
    const { book, format } = await request.json() as { book: Book; format: "pdf" | "docx" };
    ...
    if (readiness.exportReadinessStatus === "blocked") return NextResponse.json({ error: "Export blocked until manuscript readiness issues are resolved.", readiness }, { status: 409 });
